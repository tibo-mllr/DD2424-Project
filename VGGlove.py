...
image_transforms = torchvision.transforms.Compose(
    [   
        ...
        torchvision.transforms.RandomAffine(degrees = 0, translate = (0.1, 0.1)), # degrees = 0 to deactivate rotation. 
...
def train(dataloader, model, loss_fn, optimizer, device, num_epoch = None, scheduler = None):
    """
...
class WarmUpScheduler(lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        if self.last_epoch < self.warmup_steps:
            # the learning 
            return [self.initial_lr * (self.last_epoch + 1) / self.warmup_steps for base_lr in self.base_lrs]
        else:
...
if __name__ == "__main__":
  ...
  #  optimizer = torch.optim.SGD(model.parameters(), lr=1e-3, momentum=0.9, weight_decay= 0.001)
   ...
   # optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    ...
    cosine_scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs - warmupSteps)
    scheduler = lr_scheduler.SequentialLR(optimizer, schedulers = [warmup_scheduler, cosine_scheduler], milestones = [warmupSteps])
 ...
 #   scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0 = 10, T_mult = 1, eta_min = 0.001)
    ...
    for t in range(epochs):
        ...
        train_Acc, train_Loss, scheduler = train(train_dataloader, model, loss_fn, optimizer, device, t, scheduler)
